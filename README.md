...
Motor genérico para transformar vídeos-fonte em cortes verticais virais, com
um loop de agentes que aprende padrões a partir de referências e de
performance medida — sem hardcoding de nicho: tudo entra por
`niches/<niche>/config.yaml`.

cutgen é a generalização open source de um pipeline de produção real (um
canal de Shorts que já roda em produção). O mecanismo é público; os dados,
padrões e números específicos de qualquer canal não são — veja
...
`niches/example-niche/` é um esqueleto fictício — mostra o formato esperado, não é um nicho de verdade. Copie a pasta e preencha com os valores do seu próprio canal.
...
5. **render** (`cutgen_core.render`) — gera a legenda karaokê (`.ass`) e monta/roda o comando `ffmpeg` (recorte + legenda + normalização de áudio). Layout único e simples por enquanto — ver "O que não está aqui" abaixo.
